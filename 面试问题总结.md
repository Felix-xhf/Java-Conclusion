...
1. **分配对象空间**，并将对象中成员初始化为**0或者null**，因为java不允许用户操作一个不定值对象
2. 执行**属性值**的**显示初始化**
3. **执行构造器**
4. 将变量**关联**到**堆中的对象上**
...
| ---------- | ------------------------------------------------------------ | -------------------------------------- |
| 发生范围   | 同一个类                                                     | 子类                                   |
| 参数列表   | **不同**（参数类型、个数、顺序不同），返回值和访问修饰符无关。重载的方法不能根据返回类型进行区分 | 方法名、参数列表必须全部相同           |
...
```java
String s1 = new String("atguigu");String s2 = new String("atguigu");System.out.println(s1 == s2);  //false
```



```java
String s1 = "atguigu";String s2 = "atguigu";System.out.println(s1 == s2);  //true
```
...
**finalize**：垃圾回收机制中  如果想被回收的时候不是真的被回收，第二次被gc回收的时候才能回收。finalize()只会在对象内存回收前被调用一次
...
![image-20210816125715094](面试问题总结.assets/image-20210816125715094.png)



### 1.23 ==集合==—Collection VS Map
...
首先要知道java内存模型。一般情况下，如果读取主内存，他会把数据放到工作内存中（本地内存），这个时候就会出现不一致性，变量标识为volatile的话，可以告诉JVM 这个变量是不稳定的，每次用的话都必须从主内存中去取。来保证**一致性**。

然后，特性：==可见性，不保证原子性，有序性==

- 可见性：在线程解锁之前，要将共享变量的值刷新回主内存，然后线程在读取线程中的值的时候，会读取最新的值到自己的工作内存中，这样就能保证到每一个线程获取到的值是最新的。保证可见性的技术是==内存屏障==：可以使得当前工作内存中的值失效，去主内存中重新读取值。
- 不保证原子性，不能保证并发原子性
- 有序性，禁止指令重排序
...
- volatile是线程同步的轻量级的实现，性能更好的一点，但是只能修饰变量
- synchronized可以作用于方法，代码块
- 多线程访问volatile修饰的变量的时候是不会发生阻塞，synchronized是会发生阻塞的
- volatile保证可见性，不保证原子性
- volatile多用于线程间的可见性，synchronized主要是用于多线程间访问资源的同步性
...
### 4.6 Sychronized VS ReentrantLock
...
- ThreadPoolExecutor.==AbortPolicy==:**丢弃**任务并**抛出**RejectedExecutionException**异常**

- ThreadPoolExecutor.==DiscardPolicy==：**丢弃任务**，但是**不抛出异常**

- ThreadPoolExecutor.==DiscardOldestPolicy==：**丢弃**队列**最前面**的任务，然后**重新提交**被拒绝的任务
- ThreadPoolExecutor.==CallerRunsPolicy==：由调用线程（提交任务的线程）处理该任务

其中线程池**默认**的拒绝策略是**AbortPolicy**
...
==应用背景：==由于疫情的影响，很多时候我们需要在家学习，不仅是学生这样，很多成年人也需要在家进行网上学习和自我提升，所以我和同伴一起开发了一个基于SprintBoot的移动端教育课堂项   	目。这个项目是前后端分离的项目，我主要负责的是后端接口的开发工作。
...
![image-20210909101311472](面试问题总结.assets/image-20210909101311472.png)
...
MD5的固定长度为128位（16字节），可以输出==32个字符==的长度
...
- ==跨域问题==：跨域是指浏览器在发起网络请求的时候，会检查该请求所对应的协议、域名、端口和当前网页是否一致，如果不一致则浏览器会进行限制。比如登录了 www.baidu.com 并且登陆了自己的信息，然后使用ajax去访问www.jd.com是不行的，session中存有登录百度的个人信息，可以会将百度的登录信息泄露给京东，如果京东网站换成是一个黑客的网站，就会泄露自己的安全信息。
...
```java
	//失效时间,一周    private static final long EXPIRE = 60000 * 60 * 24 * 7;    //加密秘钥    private static final String SECRET = "xdclass.net168";    //令牌前缀    private static final String TOKEN_PREFIX = "xdclass";    //颁布者    private static final String SUBJECT = "xdclass";
```

**生成token：**

```java
String token = Jwts.builder().setSubject(SUBJECT)  // 颁布者                .claim("head_img",user.getHeadImg())  //claim：业务的信息，个人信息                .claim("id",user.getId())                .claim("name",user.getName())                .setIssuedAt(new Date())  //生成的日期                .setExpiration(new Date(System.currentTimeMillis() + EXPIRE)) //持续的时间                .signWith(SignatureAlgorithm.HS256,SECRET).compact(); //生成的算法 HS256        token = TOKEN_PREFIX + token;        return token;
```
...
```
注意点：*  1. 生成的token是可以通过base64进行解密出明文信息的*  2. base64进行解密出明文信息，修改在进行编码，则会解密失败*  3. 无法作废已经颁布的令牌token，除非改秘钥
```
...
过期时间到了，热点数据会大量的去mysql造成什么影响、怎么解决
...
- 工厂模式：和简单工厂模式中工厂负责生产所有产品相比，工厂方法模式将生成具体产品的任务分发给具体的产品工厂
